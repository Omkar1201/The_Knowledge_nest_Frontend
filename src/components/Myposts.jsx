...
export default function Myposts({ data }) {
    ...
    const saveChanges = async () => {
        ...
        try {
            const response = await fetch(`https://the-knowledge-nest-server.onrender.com/api/v1/editpost`, {
                method: 'PUT',
                headers: {
    ...
    async function Deletepost(post_id) {
        ...
        setbtnloading(true)
        try {
            const response = await fetch(`https://the-knowledge-nest-server.onrender.com/api/v1/deletepost`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
